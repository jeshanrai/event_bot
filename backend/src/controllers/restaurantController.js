...
const updateSettings = async (req, res) => {
    try {
        ...
        console.log(`[updateSettings] User ID: ${req.user.id}, Restaurant ID: ${restaurantId}, Currency: ${currency}`);

        if (!restaurantId) {
            console.error('[updateSettings] No restaurant ID found on user object');
            return res.status(400).json({ message: 'User is not linked to a restaurant' });
        }
        ...
        const updatedRestaurant = await Restaurant.updateCurrency(restaurantId, currency);

        if (!updatedRestaurant) {
            console.error(`[updateSettings] Restaurant not found for ID: ${restaurantId}`);
            return res.status(404).json({ message: 'Restaurant not found' });
        }
