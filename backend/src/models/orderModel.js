...
const Order = {
    create: async (orderRequest) => {
        ...
        try {
            ...
            const { service_type, payment_method, total_amount, platform, restaurant_id } = orderData;
          ...
          `;
            ...
            const orderValues = [user_id, targetRestaurantId, platform || 'whatsapp', 'created', service_type || 'dine_in', payment_method, total_amount];
          ...
          `;
            ...
            for (const item of items) {
                await client.query(itemQuery, [newOrder.id, item.food_id, item.quantity, item.unit_price]);
            }
