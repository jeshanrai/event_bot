...
7. When user confirms order (process_order_response action='confirmed') → call show_dine_in_payment_options (Order is implicitly Dine-in)
8. (Skipped - Delivery removed)
9. (Skipped - Delivery removed)
10. When order confirmed → call show_dine_in_payment_options
...
- If context stage='selecting_payment' and user mentions ANY payment preference → YOU MUST call the appropriate payment tool:
  - User says "online" OR "online payment" OR "stripe" OR "card" → call pay_online
  - User says "cash" OR "cod" OR "cash on delivery" OR "cash payment" → call pay_cash_counter
  - For dine-in cash payment at counter → call pay_cash_counter
